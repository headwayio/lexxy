...
test("drag wrapped H3 to nest inside a list item", async ({ editor, page }) => {
  ...
  // Simulate Section E from the first screenshot
  await editor.setValue([
    '<ul>',
    '<li>Section E</li>',
    '<li class="lexxy-nested-listitem"><ul>',
  ...
  // Drag the H3's li to nest inside "Item above"
  ...
  // Drop "inside" Item above (center zone)
  const targetBox = await itemAbove.boundingBox()
  await page.mouse.move(targetBox.x + targetBox.width / 2, targetBox.y + targetBox.height / 2, { steps: 10 })
  await page.waitForTimeout(100)

  const indicatorVisible = await page.locator(".lexxy-drop-indicator--visible").isVisible()
  console.log("Indicator visible for H3 nest:", indicatorVisible)
  ...
  const result = await editor.value()
  console.log("H3 RESULT:", result)

  // H3 should now be nested inside Item above's subtree
...
test("drag wrapped H3 to root level after list", async ({ editor, page }) => {
  ...
  // Drag the H3 to after "Padding 1" (root level)
  const h3Li = editor.content.locator("li:has(> h3)")
  ...
  const result = await editor.value()
  console.log("H3 ROOT RESULT:", result)

  // H3 should be at root level, after Padding 1
